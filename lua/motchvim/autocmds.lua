...
autocmd("LspAttach", {
  ...
  callback = function(args)
    ...
    local map_opts = function(override)
      return vim.tbl_extend("force", { buffer = bufnr, silent = true }, override or {})
    end
    ...
    vim.keymap.set("n", "<leader>cc", vim.lsp.codelens.run, map_opts { desc = "Run codelens" })
    ...
    if client and client.name == "Next LS" then
      vim.keymap.set(
        ...
        map_opts { desc = "Convert nested syntax to pipe syntax" }
      )
      ...
      vim.keymap.set(
        ...
        map_opts { desc = "Convert pipe syntax to nested syntax" }
      )
    end
