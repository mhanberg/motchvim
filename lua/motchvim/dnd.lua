...
dnd.previous_session = function()
  ...
  if previous_session_err then
    vim.api.nvim_err_write(
      "[zk] Error fetching the previous session's note: " .. vim.inspect(previous_session_err)
    )
    return
  end
...
vim.cmd([[command! InsertPreviousSessionLink lua motchvim.dnd.insert_link(motch.dnd.previous_session())]])
